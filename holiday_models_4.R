  ...
  if(isholiday==0){
    ...
    IV_vec <-  ((1-para[,1])*D1bar + original_price*D2barnh -
                  (exp(cnh)/(para[,3]-hollen))*((D1bar- D1+D2barnh)^2+
                                                  exp(2*(ThetaAll[2] + ThetaAll[6]*para[,2])) + exp(2*ThetaAll[3])) +
                  original_price*D2 -
                  ((exp(ThetaAll[1])+exp(cnh))/hollen)*((D1+D2)^2++exp(2*(ThetaAll[2] + ThetaAll[6]*para[,2]))+exp(2*ThetaAll[2])))/(D1bar)
...
dealloglik <- function(Theta_est,Theta_known,para,deal_data,isholiday,IV,normalize,Theta_est_nh){
  ...
  if(isholiday==0){
    ...
    ThetaAll <- c(Theta_est[1],Theta_known[28],Theta_known[45],Theta_known[46],Theta_est_nh[6],Theta_est[10])
    #ThetaAll_nh <- list(Theta_est_nh[1],Theta_known[1:28],c(Theta_est_nh[2:6],Theta_known[13:25]),c(Theta_est_nh[8],Theta_est_nh[7]))
    ...
    numer <- (discprice*D1bar + original_price*D2barnh -
                (exp(cnh)/(para[,3]-hollen))*((D1bar- D1barh+ D2barnh)^2+
                                                exp(2*(ThetaAll[2] + Theta_est[10]*para[,2])) + exp(2*ThetaAll[3])) +
                original_price*D2barh -
                ((exp(Theta_est[1])+exp(cnh))/hollen)*((D1barh+D2barh)^2+exp(2*(ThetaAll[2] + Theta_est[10]*para[,2]))+exp(2*ThetaAll[2])))/(D1bar)
...
max_loglik <- function(deal_data,sales_estimates,sales_estimates_h,isholiday,Theta_est_nh,siggam){
  ...
  if(isholiday == 0){
    ...
    Theta_est <- c(Theta_est_nh[1],-log(log(abs(sales_estimates_h[c(4)]))),
                   ...
                   sign(sales_estimates_h[c(6)])*log(abs(sales_estimates_h[c(6)])),gamh)
    Theta_est <- Theta_est + runif(10,-2,2)*abs(Theta_est)
