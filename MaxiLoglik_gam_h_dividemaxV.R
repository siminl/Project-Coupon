...
dealloglik <- function(Theta_est,Theta_known,para,deal_data,isholiday,IV,normalize,Theta_est_nh){
  ...
  if(isholiday==0){
    ...
    D2barh <- exp(-exp(Theta_est[5])*log(para[,3])+((Theta_est[7])))
...
sample_loglik <- function(Theta_est,Theta_known,deal_data,isholiday,Theta_est_nh){
  ...
  if(isholiday==0){
    ...
    for(i in 1:dim(deal_data)[1]){
      ...
      D2hbar_t <- exp(-exp(Theta_est[5])*log(para_choice[,3])+((Theta_est[7])))
...
max_loglik <- function(deal_data,sales_estimates,sales_estimates_h,isholiday,Theta_est_nh,siggam){
  ...
  if(isholiday == 0){
    ...
    Theta_est <- c(Theta_est_nh[1],-log(log(abs(sales_estimates_h[c(4)]))),
                   ...
                   sign(sales_estimates_h[c(2)])*log(log(abs(sales_estimates_h[c(2)]))),
...
(exp(-exp(mleout_h_ms[[6]][[5]]$par[2])*hol_data$original_price*(1-hol_data$discount)+mleout_h_ms[[6]][[5]]$par[3]*log(hol_data$prep.period)+((mleout_h_ms[[6]][[5]]$par[4]))*log(hol_data$offering_duration)+mleout_h_ms[[6]][[5]]$par[6])/
    (1+exp(-exp(mleout_h_ms[[6]][[5]]$par[2])*hol_data$original_price*(1-hol_data$discount)+mleout_h_ms[[6]][[5]]$par[3]*log(hol_data$prep.period)+((mleout_h_ms[[6]][[5]]$par[4]))*log(hol_data$offering_duration)+mleout_h_ms[[6]][[5]]$par[6])))

exp(-exp(mleout_h_ms[[6]][[5]]$par[5])*log(hol_data$offering_duration)+(-exp(mleout_h_ms[[6]][[5]]$par[7])))
