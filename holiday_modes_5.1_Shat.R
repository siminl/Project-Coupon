...
dealloglik <- function(Theta_est,Theta_known,para,deal_data,isholiday,IV,normalize,Theta_est_nh,sales_est,sales_est_h,maxS){
  ...
  if(isholiday==0){
    ...
    D1bar <- (deal_data$total_volume/exp(sales_est_h[4]*(1-deal_data$discount)*original_price +
                                           ...
                                           1.07*log(deal_data$prep.period)))*
    ...
    D1barh <- D1bar*(((Theta_est[2])*discprice+((Theta_est[3]))*log(para[,2])+(-exp(Theta_est[4]))*log(para[,3])+Theta_est[6])/
                       (1+((Theta_est[2])*discprice+((Theta_est[3]))*log(para[,2])+(-exp(Theta_est[4]))*log(para[,3])+Theta_est[6])))
    
    D2barh <- exp((Theta_est[5])*log(para[,3])+((Theta_est[7])))
  ...
  if(isholiday==0){
    ...
    for(i in 1:dim(deal_data)[1]){
      ...
      D1bar <- (total_volume/exp(sales_est_h[4]*(1-deal_data$discount[i])*deal_data$original_price[i] +
                                   1.24*log(deal_data$offering_duration[i]) +
                                   1.07*log(deal_data$prep.period[i])))*
        exp(sales_est_h[4]*(1-para_choice[,1])*deal_data$original_price[i] +
              1.24*log(para_choice[,3]) +
              1.07*log(para_choice[,2]))
      
      # D1bar <- total_volume
      
      D2nhbar_t <- exp((Theta_est_nh[2])+((Theta_est_nh[4]))*(1-para_choice[,1])*deal_data$original_price[i]+
      ...
      D1hbar_t <- D1bar*((Theta_est[2])*((1-para_choice[,1])*deal_data$original_price[i])+((Theta_est[3]))*log(para_choice[,2])+(-exp(Theta_est[4]))*log(para_choice[,3])+Theta_est[6] )/
        (1+((Theta_est[2])*((1-para_choice[,1])*deal_data$original_price[i])+((Theta_est[3]))*log(para_choice[,2])+(-exp(Theta_est[4]))*log(para_choice[,3])+Theta_est[6] ))
      
      D2hbar_t <- exp((Theta_est[5])*log(para_choice[,3])+((Theta_est[7])))
