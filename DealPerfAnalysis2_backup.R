...
for(i in industries_to_check){
  ...
  if(i %in% c(2)){
    Spri[[i]] <- (hol_data$total_volume/exp(sales_estimates_h[i,4]*(1-hol_data$discount)*hol_data$original_price +
                                              ...
                                              1.23*log(hol_data$prep.period+1))*
    ...
    Spri[[i]] <- (hol_data$total_volume/exp(sales_estimates_h[i,4]/10*(1-hol_data$discount)*hol_data$original_price +
                                              ...
                                              0.124*log(hol_data$prep.period))*
  ...
  if(i %in% c(1)){
    ...
    # D2hvalpri[[i]] <- exp(-exp(mleout_h[[i]]$par[5])*log(hol_data$holiday.len)+((mleout_h[[i]]$par[7]))+
    #                         (log(D2nhvalpri[[i]])))
  ...
  dalter[[i]] <- (-(as.numeric(Spri[[i]] + pmax(D2nhvalpri[[i]],0)+ pmax(D2hvalpri[[i]],0))) +
    ...
    (abs(as.numeric(Spri[[i]] + pmax(D2nhvalpri[[i]],0 )+ pmax(D2hvalpri[[i]],0))))
  
  nondealprofit[[i]] <- hol_data$original_price*Spri[[i]] + 
    hol_data$original_price*((D2nhvalpri[[i]]) + (D2hvalpri[[i]])) - 
    ch*(((D2nhvalpri[[i]])+(nondealdemandnhol))^2+exp(gamma_h[i,1]*2)+exp(gamma_preh[i,1]*2)) -
    ch*(((D2hvalpri[[i]]) + (nondealdemandhol))^2+exp(gamma_h[i,1]*2)+exp(gamma_h[i,1]*2))
  ...
  profitalter[[i]] <- (nondealprofit[[i]]-dealprofit[[i]])/abs(dealprofit[[i]])
  ...
  print(paste("sig alterD = ", sd(dalter[[i]][dalter[[i]]<quantile(dalter[[i]],0.8)&
                                                dalter[[i]]>quantile(dalter[[i]],0.2)])))
  print(paste("alterP = ", mean(profitalter[[i]][profitalter[[i]]<quantile(profitalter[[i]],0.75)&
                                                   profitalter[[i]]>quantile(profitalter[[i]],0.25)])))
  print(paste("sig alterP = ", sd(profitalter[[i]][profitalter[[i]]<quantile(profitalter[[i]],0.75)&
                                                     profitalter[[i]]>quantile(profitalter[[i]],0.25)])))
...
for(i in industries_to_check){
  ...
  if(i %in% c(5)){
    ...
    nonholnormdemand[[i]] <- D2nhvalprid2[[i]] + nonhol_data$total_volume/exp(coef((summary(lm_nonhol)))[4]/10*(1-nonhol_data$discount)*nonhol_data$original_price +
                                                                                coef((summary(lm_nonhol)))[2]/10*log(nonhol_data$offering_duration))*
      exp(coef((summary(lm_nonhol)))[4]/10*nonhol_data$original_price)
