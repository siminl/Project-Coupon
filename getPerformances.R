...
getPerformance <- function(hol_data,sdop, nonhol_data, sdopph, output_ls, type,mleout_preh,mleout_h){
  ...
  for(kiter in 1:length(output_ls)){
    ...
    Spri[[kiter]] <- (hol_data$total_volume/exp(Scoef[Scoef$type==type,2]*(1-hol_data$discount)*hol_data$original_price +
      ...
      exp(Scoef[Scoef$type==type,2]*hol_data$original_price))*hol_data$offering_duration
    ...
    if(type%in%c(1,2)){
      ...
      D1hvalpri[[kiter]] <- Stotpri*(((output_ls[[kiter]]$par[4]))*log(hol_data$holiday.len+1)+output_ls[[kiter]]$par[6]+ exp(fes_h)/(hol_data$total_volume-exp(fes_h)))/
        (1+(((output_ls[[kiter]]$par[4]))*log(hol_data$holiday.len+1)+output_ls[[kiter]]$par[6]+ exp(fes_h)/(hol_data$total_volume-exp(fes_h))))
      ...
      D2nhvalpri[[kiter]] <- ((mleout_preh[[type]]$par[2])+
                            -exp(mleout_preh[[type]]$par[3])*log(hol_data$holiday.len+1))*exp(as.numeric(fes))
      
      
      D2hval[[kiter]] <- exp((output_ls[[kiter]]$par[5])*log(hol_data$offering_duration)+((output_ls[[kiter]]$par[7]))+
                           (D2nhval[[kiter]]))
      
      D2hvalpri[[kiter]] <- exp((output_ls[[kiter]]$par[5])*log(hol_data$holiday.len+1)+((output_ls[[kiter]]$par[7]))+
                              (D2nhvalpri[[kiter]]))
      ...
      D1hval[[kiter]] <- Stot*((output_ls[[kiter]]$par[2])*((1-hol_data$discount)*hol_data$original_price)+((output_ls[[kiter]]$par[3]))*log(hol_data$prep.period)+(-exp(output_ls[[kiter]]$par[4]))*log(hol_data$offering_duration)+output_ls[[kiter]]$par[6]+ exp(fes_h)/(hol_data$total_volume-exp(fes_h)))/
      ...
      D1hvalpri[[kiter]] <- Stotpri*((-exp(output_ls[[kiter]]$par[4]))*log(hol_data$holiday.len+1)+output_ls[[kiter]]$par[6])/
        (1+((-exp(output_ls[[kiter]]$par[4]))*log(hol_data$holiday.len+1)+output_ls[[kiter]]$par[6]))
      ...
      if( type == 5){
        ...
        D2hvalpri[[kiter]] <- ((output_ls[[kiter]]$par[5])*log(hol_data$holiday.len+1)+((output_ls[[kiter]]$par[7])))
    ...
    # 
    # print(paste("dsidx = ", mean(dsidx[[kiter]][dsidx[[kiter]]<quantile(dsidx[[kiter]],0.9)&
    #                                           dsidx[[kiter]]>quantile(dsidx[[kiter]],0.1)])," ",
    #             sd(dsidx[[kiter]][dsidx[[kiter]]<quantile(dsidx[[kiter]],0.9)&
    #                             dsidx[[kiter]]>quantile(dsidx[[kiter]],0.1)])))
    # print(paste("dsidx2 = ", mean(dsidx2[[kiter]][dsidx2[[kiter]]<quantile(dsidx2[[kiter]],0.8)&
    #                                             dsidx2[[kiter]]>quantile(dsidx2[[kiter]],0.2)])," ",
    #             sd(dsidx2[[kiter]][dsidx2[[kiter]]<quantile(dsidx2[[kiter]],0.8)&
    #                              dsidx2[[kiter]]>quantile(dsidx2[[kiter]],0.2)])))
  
    Dtotalpri[[kiter]] <- as.numeric(Spri[[kiter]] + D2nhvalpri[[kiter]] + D2hvalpri[[kiter]])

    
    dalter[[kiter]] <- (-(as.numeric(Spri[[kiter]] + pmax(D2nhvalpri[[kiter]],0)+ pmax(D2hvalpri[[kiter]]/(hol_data$holiday.len+1),0))) +
                      as.numeric(hol_data$total_volume + pmax(0,D2nhval[[kiter]])+ pmax(0,D2hval[[kiter]])))/
      ((as.numeric(Spri[[kiter]] + pmax(D2nhvalpri[[kiter]],0)+ pmax(D2hvalpri[[kiter]]/(hol_data$holiday.len+1),0))))
    
    nondealprofit[[kiter]] <- hol_data$original_price*Spri[[kiter]]*sd(sdop$original_price) +
      hol_data$original_price*sd(sdop$original_price)*((D2nhvalpri[[kiter]]) + (D2hvalpri[[kiter]]/(hol_data$holiday.len+1))) -
      ...
      ch*sd(sdop$original_price)/(hol_data$holiday.len)*(((D2hvalpri[[kiter]]/(hol_data$holiday.len+1)) + (nondealdemandhol))^2+exp(gamma_h[i,1]*2)+exp(gamma_h[i,1]*2))
    ...
    nonholnormdemand[[kiter]] <- pmax(D2nhndvalpri[[kiter]],0) + 
    ...
    holnormdemand[[kiter]] <- (as.numeric(Spri[[kiter]] + 
                                        pmax(D2nhvalpri[[kiter]]/(hol_data$holiday.len+1)*hol_data$offering_duration,0) + 
                                        pmax(D2hvalpri[[kiter]]/(hol_data$holiday.len+1)*hol_data$offering_duration,0)))/hol_data$offering_duration 
    ...
    muh <- mean(holnormdemand[[kiter]][holnormdemand[[kiter]]<quantile(holnormdemand[[kiter]],ub,na.rm = TRUE)&
                                     holnormdemand[[kiter]]>quantile(holnormdemand[[kiter]],lb,na.rm = TRUE)])
    munh <- mean(nonholnormdemand[[kiter]][nonholnormdemand[[kiter]]<quantile(nonholnormdemand[[kiter]],ub,na.rm = TRUE)&
  ...
  return(list(  mchs = mchs, mcnhs=mcnhs,
                ...
                dsidx  = dsidx,dsidx2 = dsidx2,
                dalter = dalter,Dtotalpri = Dtotalpri,
...
for(i in industries_to_check){
  ...
  ch <- (exp((output_ls[[i]]$par[1])))
  cnh <- (exp((mleout_preh[[i]]$par[1])))
  cvec <- c(cvec,ch)
  cnhvec <- c(cnhvec,cnh)
  #print(paste(output_ls[[i]]$par[1],mleout_preh[[i]]$par[1]))
  chs <- c(chs, (exp((output_ls[[i]]$par[1])))*sd(sdop$original_price))
  cnhs <- c(cnhs,(exp((mleout_preh[[i]]$par[1])))*sd(sdopph$original_price))
  print(paste("mch = ",(exp((output_ls[[i]]$par[1])))*sd(sdop$original_price)))
  print(paste("mcph = ",(exp((mleout_preh[[i]]$par[1])))*sd(sdopph$original_price)))
  
  muc <- (exp((output_ls[[i]]$par[1])))*sd(sdop$original_price)
  mucp <- mean(muc/sdop$original_price[sdop$original_price<quantile(sdop$original_price,0.9)&
                                         sdop$original_price>quantile(sdop$original_price,0.1)])
  ...
  if(i %in% c(2)){
    Spri[[i]] <- (hol_data$total_volume/exp(-4.47*(1-hol_data$discount)*hol_data$original_price +
                                              3.36*log(hol_data$offering_duration) + 
                                              2.60*(hol_data$prep.period)
    )*
      exp(-4.47*hol_data$original_price))*hol_data$offering_duration
    ...
    Spri[[i]] <- (hol_data$total_volume/exp(-1.40*(1-hol_data$discount)*hol_data$original_price +
                                              6.33*log(hol_data$offering_duration) + 
                                              0.88*log(hol_data$prep.period)
    )*
      exp(-1.40*hol_data$original_price))*hol_data$offering_duration
    
    
    
    
  }else if(i %in% c(10)){
    Spri[[i]]  <- (hol_data$total_volume/exp(-1.4*(1-hol_data$discount)*hol_data$original_price +
                                               # 1.7*log(hol_data$offering_duration) + 
                                               0.69*(hol_data$prep.period))*
                     exp(-1.4*hol_data$original_price))*hol_data$offering_duration
    
    
  }else if(i %in% c(1)){
    
    Spri[[i]]  <-  (hol_data$total_volume/exp(-3.2*(1-hol_data$discount)*hol_data$original_price +
                                                0.04*log(hol_data$offering_duration)+
                                                1.03*(hol_data$prep.period)
    )*
      exp(-3.2*hol_data$original_price))*hol_data$offering_duration
    ...
    Spri[[i]]  <- (hol_data$total_volume/exp(-3.01*(1-hol_data$discount)*hol_data$original_price +
                                               1.0*log(hol_data$offering_duration)+
                                               2.8*(hol_data$prep.period))*
                     exp(-3.01*hol_data$original_price))*hol_data$offering_duration
  ...
  if(i %in% c(1)){
    ...
    # D1hval[[i]] <- hol_data$total_volume*((output_ls[[i]]$par[2])*((1-hol_data$discount)*hol_data$original_price)+((output_ls[[i]]$par[3]))*log(hol_data$prep.period)+((output_ls[[i]]$par[4]))*log(hol_data$offering_duration)+output_ls[[i]]$par[6]+ exp(fes_h)/(hol_data$total_volume-exp(fes_h)))/
    #   (1+((output_ls[[i]]$par[2])*((1-hol_data$discount)*hol_data$original_price)+((output_ls[[i]]$par[3]))*log(hol_data$prep.period)+((output_ls[[i]]$par[4]))*log(hol_data$offering_duration)+output_ls[[i]]$par[6]+ exp(fes_h)/(hol_data$total_volume-exp(fes_h))))
    # 
    # D1hvalpri[[i]] <- hol_data$total_volume*(output_ls[[i]]$par[6]+ exp(fes_h)/(hol_data$total_volume-exp(fes_h)))/
    #   (1+(output_ls[[i]]$par[6]+ exp(fes_h)/(hol_data$total_volume-exp(fes_h))))
    # 
    
    D1hval[[i]] <- Stot*((output_ls[[i]]$par[2])*((1-hol_data$discount)*hol_data$original_price)+((output_ls[[i]]$par[3]))*log(hol_data$prep.period)+((output_ls[[i]]$par[4]))*log(hol_data$offering_duration)+output_ls[[i]]$par[6]+ exp(fes_h)/(hol_data$total_volume-exp(fes_h)))/
      (1+((output_ls[[i]]$par[2])*((1-hol_data$discount)*hol_data$original_price)+((output_ls[[i]]$par[3]))*log(hol_data$prep.period)+((output_ls[[i]]$par[4]))*log(hol_data$offering_duration)+output_ls[[i]]$par[6]+ exp(fes_h)/(hol_data$total_volume-exp(fes_h))))
    
    D1hvalpri[[i]] <- Stotpri*(((output_ls[[i]]$par[4]))*log(hol_data$holiday.len+1)+output_ls[[i]]$par[6]+ exp(fes_h)/(hol_data$total_volume-exp(fes_h)))/
      (1+(((output_ls[[i]]$par[4]))*log(hol_data$holiday.len+1)+output_ls[[i]]$par[6]+ exp(fes_h)/(hol_data$total_volume-exp(fes_h))))
    
    
    r[[i]] <- ((output_ls[[i]]$par[2])*((1-hol_data$discount)*hol_data$original_price)+((output_ls[[i]]$par[3]))*log(hol_data$prep.period)+((output_ls[[i]]$par[4]))*log(hol_data$offering_duration)+output_ls[[i]]$par[6]+ exp(fes_h)/(hol_data$total_volume-exp(fes_h)))/
      (1+((output_ls[[i]]$par[2])*((1-hol_data$discount)*hol_data$original_price)+((output_ls[[i]]$par[3]))*log(hol_data$prep.period)+((output_ls[[i]]$par[4]))*log(hol_data$offering_duration)+output_ls[[i]]$par[6]+ exp(fes_h)/(hol_data$total_volume-exp(fes_h))))    
    
    thetaalpha <- c(thetaalpha,(output_ls[[i]]$par[2]))
    thetaT<- c(thetaT,((output_ls[[i]]$par[3])))
    thetad<- c(thetad,((output_ls[[i]]$par[4])))
    
    
    
  }else if (i %in%c(2)){
    # D1hval[[i]] <- hol_data$total_volume*((output_ls[[i]]$par[2])*((1-hol_data$discount)*hol_data$original_price)+((output_ls[[i]]$par[3]))*log(hol_data$prep.period)+(-exp(output_ls[[i]]$par[4]))*log(hol_data$offering_duration)+output_ls[[i]]$par[6])/
    #   (1+((output_ls[[i]]$par[2])*((1-hol_data$discount)*hol_data$original_price)+((output_ls[[i]]$par[3]))*log(hol_data$prep.period)+(-exp(output_ls[[i]]$par[4]))*log(hol_data$offering_duration)+output_ls[[i]]$par[6]))
    # 
    # D1hvalpri[[i]] <- hol_data$total_volume*(+output_ls[[i]]$par[6])/
    #   (1+(output_ls[[i]]$par[6]))
    
    # r[[i]] <- ((output_ls[[i]]$par[2])*((1-hol_data$discount)*hol_data$original_price)+((output_ls[[i]]$par[3]))*log(hol_data$prep.period)+(-exp(output_ls[[i]]$par[4]))*log(hol_data$offering_duration)+output_ls[[i]]$par[6])/
    #   (1+((output_ls[[i]]$par[2])*((1-hol_data$discount)*hol_data$original_price)+((output_ls[[i]]$par[3]))*log(hol_data$prep.period)+(-exp(output_ls[[i]]$par[4]))*log(hol_data$offering_duration)+output_ls[[i]]$par[6]))
    # 
    ...
    # D1hvalpri[[i]] <- Stotpri*(((output_ls[[i]]$par[3]))*log(hol_data$prep.period)+(-exp(output_ls[[i]]$par[4]))*log(hol_data$offering_duration)+output_ls[[i]]$par[6] + exp(fes_h)/(hol_data$total_volume-exp(fes_h)))/
    #   (1+(((output_ls[[i]]$par[3]))*log(hol_data$prep.period)+(-exp(output_ls[[i]]$par[4]))*log(hol_data$offering_duration)+output_ls[[i]]$par[6] + exp(fes_h)/(hol_data$total_volume-exp(fes_h))))
    
    D1hvalpri[[i]] <- Stotpri*(((output_ls[[i]]$par[4]))*log(hol_data$holiday.len+1)+output_ls[[i]]$par[6] + exp(fes_h)/(hol_data$total_volume-exp(fes_h)))/
      (1+(((output_ls[[i]]$par[4]))*log(hol_data$holiday.len+1)+output_ls[[i]]$par[6] + exp(fes_h)/(hol_data$total_volume-exp(fes_h))))
    
    
    D1hvalprid1[[i]] <- Stot*(((output_ls[[i]]$par[3]))*log(2)+output_ls[[i]]$par[6] + exp(fes_h)/(hol_data$total_volume-exp(fes_h)))/
      (1+(((output_ls[[i]]$par[3]))*log(2)+output_ls[[i]]$par[6] + exp(fes_h)/(hol_data$total_volume-exp(fes_h))))
    
    
    r[[i]] <- ((output_ls[[i]]$par[2])*((1-hol_data$discount)*hol_data$original_price)+((output_ls[[i]]$par[3]))*log(hol_data$prep.period)+((output_ls[[i]]$par[4]))*log(hol_data$offering_duration)+output_ls[[i]]$par[6])/
      (1+((output_ls[[i]]$par[2])*((1-hol_data$discount)*hol_data$original_price)+((output_ls[[i]]$par[3]))*log(hol_data$prep.period)+((output_ls[[i]]$par[4]))*log(hol_data$offering_duration)+output_ls[[i]]$par[6]))
    ...
    D1hval[[i]] <- Stot*((output_ls[[i]]$par[2])*((1-hol_data$discount)*hol_data$original_price)+((output_ls[[i]]$par[3]))*log(hol_data$prep.period)+(-exp(output_ls[[i]]$par[4]))*log(hol_data$offering_duration)+output_ls[[i]]$par[6])/
      (1+((output_ls[[i]]$par[2])*((1-hol_data$discount)*hol_data$original_price)+((output_ls[[i]]$par[3]))*log(hol_data$prep.period)+(-exp(output_ls[[i]]$par[4]))*log(hol_data$offering_duration)+output_ls[[i]]$par[6]))
    
    D1hvalpri[[i]] <- Stotpri*((-exp(output_ls[[i]]$par[4]))*log(hol_data$holiday.len+1)+output_ls[[i]]$par[6])/
      (1+((-exp(output_ls[[i]]$par[4]))*log(hol_data$holiday.len+1)+output_ls[[i]]$par[6]))
    ...
    D1hval[[i]] <- Stot*((output_ls[[i]]$par[2])*((1-hol_data$discount)*hol_data$original_price)+((output_ls[[i]]$par[3]))*log(hol_data$prep.period)+((output_ls[[i]]$par[4]))*log(hol_data$offering_duration)+output_ls[[i]]$par[6])/
      (1+((output_ls[[i]]$par[2])*((1-hol_data$discount)*hol_data$original_price)+((output_ls[[i]]$par[3]))*log(hol_data$prep.period)+((output_ls[[i]]$par[4]))*log(hol_data$offering_duration)+output_ls[[i]]$par[6]))
    
    D1hvalpri[[i]] <- Stotpri*(((output_ls[[i]]$par[4]))*log(hol_data$holiday.len+1)+output_ls[[i]]$par[6])/
      (1+(((output_ls[[i]]$par[4]))*log(hol_data$holiday.len+1)+output_ls[[i]]$par[6]))
    ...
    D1hval[[i]] <- Stot*((output_ls[[i]]$par[2])*((1-hol_data$discount)*hol_data$original_price)+((output_ls[[i]]$par[3]))*log(hol_data$prep.period)+(-exp(output_ls[[i]]$par[4]))*log(hol_data$offering_duration)+output_ls[[i]]$par[6]+ exp(fes_h)/(hol_data$total_volume-exp(fes_h)))/
      (1+((output_ls[[i]]$par[2])*((1-hol_data$discount)*hol_data$original_price)+((output_ls[[i]]$par[3]))*log(hol_data$prep.period)+(-exp(output_ls[[i]]$par[4]))*log(hol_data$offering_duration)+output_ls[[i]]$par[6]+ exp(fes_h)/(hol_data$total_volume-exp(fes_h))))
    
    D1hvalpri[[i]] <- Stotpri*((-exp(output_ls[[i]]$par[4]))*log(hol_data$holiday.len+1)+output_ls[[i]]$par[6]+ exp(fes_h)/(hol_data$total_volume-exp(fes_h)))/
      (1+((-exp(output_ls[[i]]$par[4]))*log(hol_data$holiday.len+1)+output_ls[[i]]$par[6]+ exp(fes_h)/(hol_data$total_volume-exp(fes_h))))
    
    D1hvalprid1[[i]] <- Stot*(((output_ls[[i]]$par[3]))*log(2)+output_ls[[i]]$par[6]+ exp(fes_h)/(hol_data$total_volume-exp(fes_h)))/
      (1+(((output_ls[[i]]$par[3]))*log(2)+output_ls[[i]]$par[6]+ exp(fes_h)/(hol_data$total_volume-exp(fes_h))))
    
    r[[i]] <- ((output_ls[[i]]$par[2])*((1-hol_data$discount)*hol_data$original_price)+((output_ls[[i]]$par[3]))*log(hol_data$prep.period)+(-exp(output_ls[[i]]$par[4]))*log(hol_data$offering_duration)+output_ls[[i]]$par[6]+ exp(fes_h)/(hol_data$total_volume-exp(fes_h)))/
      (1+((output_ls[[i]]$par[2])*((1-hol_data$discount)*hol_data$original_price)+((output_ls[[i]]$par[3]))*log(hol_data$prep.period)+(-exp(output_ls[[i]]$par[4]))*log(hol_data$offering_duration)+output_ls[[i]]$par[6]+ exp(fes_h)/(hol_data$total_volume-exp(fes_h))))
    
    thetaalpha <- c(thetaalpha,(output_ls[[i]]$par[2]))
    thetaT<- c(thetaT,((output_ls[[i]]$par[3])))
    thetad<- c(thetad,(-exp(output_ls[[i]]$par[4])))
  ...
  if(i %in% c(1,10)){
    ...
    D2nhvalprid1[[i]] <- ((mleout_preh[[i]]$par[2]))*exp(as.numeric(fes))
    
    fes_nh <- control_FE(sales_estimates[i,],nonhol_data)
    D2nhvalprid2[[i]] <- ((mleout_preh[[i]]$par[2])-exp(mleout_preh[[i]]$par[3])*log(2))*exp(as.numeric(fes_nh))
  ...
  if(i %in% c(1)){
    D2hval[[i]] <- exp((output_ls[[i]]$par[5])*log(hol_data$offering_duration)+((output_ls[[i]]$par[7]))+
                         (D2nhval[[i]]))
    
    D2hvalpri[[i]] <- exp((output_ls[[i]]$par[5])*log(hol_data$holiday.len+1)+((output_ls[[i]]$par[7]))+
                            (D2nhvalpri[[i]]))
    
    D2hvalprid1[[i]] <- exp((output_ls[[i]]$par[5])*log(2)+((output_ls[[i]]$par[7]))+
                              D2nhvalprid1[[i]])
    
    betad <- c(betad,(output_ls[[i]]$par[5]))
    
  }else if(i %in% c(10)){
    D2hval[[i]] <- ((output_ls[[i]]$par[5])*log(hol_data$offering_duration)+((output_ls[[i]]$par[7]))+
                      abs(D2nhval[[i]]))
    
    D2hvalpri[[i]] <- ((output_ls[[i]]$par[5])*log(hol_data$holiday.len+1)+((output_ls[[i]]$par[7]))+
                         abs(D2nhvalpri[[i]]))
    
    
    D2hvalpri[[i]] <- (((output_ls[[i]]$par[7]))+
                         (D2nhvalpri[[i]]))
    ...
    D2hvalpri[[i]] <- exp(((output_ls[[i]]$par[7]))+
                            ((D2nhvalpri[[i]])))
  ...
  #print(sd(sdop$original_price)*exp(output_ls[[i]]$par[1]))
...
for(i in industries_to_check){
  ...
  if(i %in% c(10)){
    ...
    holnormdemand[[i]] <- (as.numeric(Spri[[i]] + 
                                        pmax(D2nhvalpri[[i]]*hol_data$offering_duration,0) + 
                                        pmax(D2hvalpri[[i]]/(hol_data$holiday.len+1)*hol_data$offering_duration,0)))
