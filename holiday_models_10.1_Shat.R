...
dealloglik <- function(Theta_est,Theta_known,para,deal_data,isholiday,IV,normalize,Theta_est_nh,sales_est,sales_est_h,maxS){
  ...
  if(isholiday==0){
    ...
    D1barh <- D1bar*(((Theta_est[2])*discprice+((Theta_est[3]))*log(para[,2])+(-exp(Theta_est[4]))*log(para[,3])+Theta_est[6]+ exp(fes_h)/(deal_data$total_volume-exp(fes_h)))/
                       (1+((Theta_est[2])*discprice+((Theta_est[3]))*log(para[,2])+(-exp(Theta_est[4]))*log(para[,3])+Theta_est[6]+ exp(fes_h)/(deal_data$total_volume-exp(fes_h)))))
    
    D2barh <- ((Theta_est[5])*log(para[,3])+((Theta_est[7]))+(abs((D2barnh))))
  ...
  if(isholiday==0){
    ...
    for(i in 1:dim(deal_data)[1]){
      ...
      D1bar <- (total_volume/exp(sales_est_h[4]*(1-deal_data$discount[i])*deal_data$original_price[i] +
                                   1.7*log(deal_data$offering_duration[i]) +
                                   10.5*log(deal_data$prep.period[i])))*
        exp(sales_est_h[4]*(1-para_choice[,1])*deal_data$original_price[i] + 
              1.7*log(para_choice[,3]) +
              10.5*log(para_choice[,2]))
      ...
      D1hbar_t <- D1bar*((Theta_est[2])*((1-para_choice[,1])*deal_data$original_price[i])+((Theta_est[3]))*log(para_choice[,2])+(-exp(Theta_est[4]))*log(para_choice[,3])+Theta_est[6] + exp(fes_h)/(total_volume-exp(fes_h)))/
        (1+((Theta_est[2])*((1-para_choice[,1])*deal_data$original_price[i])+((Theta_est[3]))*log(para_choice[,2])+(-exp(Theta_est[4]))*log(para_choice[,3])+Theta_est[6] + exp(fes_h)/(total_volume-exp(fes_h))))
      
      D2hbar_t <- ((Theta_est[5])*log(para_choice[,3])+((Theta_est[7]))+(abs((D2nhbar_t))))
