...
#mleout_preh <- list()
mlefnvalue_ls <- list()
# mlefnvalue_ls_preh <- list()

for(i in industries_to_est){
  mlefnvalue <- vector()
  for(iter in 1:NumIter){
    if(mleout_preh_ms[[iter]][[i]]$convergence==0){
      mlefnvalue <- c(mlefnvalue,mleout_preh_ms[[iter]][[i]]$value)
    }else{
      mlefnvalue <- c(mlefnvalue,NA)
    }
  }
  mlefnvalue_ls_preh[[i]] <- mlefnvalue
  #mleout_h[[i]] <- mleout_h_ms[[which(mlefnvalue==min(mlefnvalue[!mlefnvalue%in%c(0,lbvalue)]))]][[i]]
  #mleout_preh[[i]] <- mleout_preh_ms[[which(mlefnvalue==min(mlefnvalue,na.rm = TRUE))]][[i]]
  print(i)
}
