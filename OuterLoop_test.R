  ...
  if(isholiday == 0){
    for(i in 1:dim(deal_data)[1]){
      ...
      tryCatch(
        ...
        min_value[i] <- minimizer_out$min_value
        argmin_value <- rbind(argmin_value,minimizer_out$deal_min$par)
        single_likelihood[i] <- pnorm(min_value[i],mean = 0, 
                                      sd= sqrt(sigmazsq(ThetaAll,parai=parai,parak=minimizer_out$deal_min$par,deal_data[i,],isholiday=0)))},
        error = function(e){
          message("* Caught an error on itertion ", i)
          print(e)
        }
...
MLE_estimation <- function(deal_data,sales_estimates,isholiday){
  if(isholiday==0){
    ...
    fn <- function(Theta_est,Theta_known,deal_data,isholiday){
      ...
      print(rt)
      print(Theta_est)
      #print(-(log(Theta_est[1])+log(Theta_est[7])+log(Theta_est[8])))
      return(rt)
    }
    ...
    parMV <- function(deal_data,sales_estimates,isholiday){
      ...
      for(i in 1:dim(deal_data)[1]){
        ...
        parmvbeta0[i] <- (sqrt(sigmazsq_v[i])*(original_price*(D2opt[i] - D2i[i]) - c(2*partSi[i]*D2i[i]/parai_v[3] - 2*partSopt[i]*D2opt[i]/para_opt[i,3])) -
                            4*c*(partSopt[i]/para_opt[i,3] - partSi[i]/parai_v[3])*(D2opt[i]/para_opt[i,3] - D2i[i]/parai_v[3])*(exp(sigs[1])^2 + exp(sigs[2])^2)*MV_numerator)/
          sigmazsq_v[i]
        ...
        parmvbetad[i] <- (sqrt(sigmazsq_v[i])*(original_price*(D2i[i]*log(parai_v[3])-D2opt[i]*log(para_opt[i,3])) -
                            ...
                            (1/(2*sqrt(sigmazsq_v[i])))*4*c*(partSopt[i]/para_opt[i,3] - partSi[i]/parai_v[3])*(D2opt[i]*log(para_opt[i,3])/para_opt[i,3] - D2i[i]*log(parai_v[3])/parai_v[3])*
        ...
        parmvbetap[i] <- (sqrt(sigmazsq_v[i])*(original_price*(D2i[i]*log(original_price)-D2opt[i]*log(original_price)) -
                            ...
                            (1/(2*sqrt(sigmazsq_v[i])))*4*c*(partSopt[i]/para_opt[i,3] - partSi[i]/parai_v[3])*(D2opt[i]*log(original_price)/para_opt[i,3] - D2i[i]*log(original_price)/parai_v[3])*
                            (exp(sigs[1])^2 + exp(sigs[2])^2)*MV_numerator)/sigmazsq_v[i]
        ...
        parmvbetawk[i] <- (sqrt(sigmazsq_v[i])*(original_price*(D2i[i]*weekends-D2opt[i]*weekends) -
                             ...
                             (1/(2*sqrt(sigmazsq_v[i])))*4*c*(partSopt[i]/para_opt[i,3] - partSi[i]/parai_v[3])*(D2opt[i]*weekends/para_opt[i,3] - D2i[i]*weekends/parai_v[3])*
                             (exp(sigs[1])^2 + exp(sigs[2])^2)*MV_numerator)/sigmazsq_v[i]
        
        parmvbetacomp[i] <- (sqrt(sigmazsq_v[i])*(original_price*(D2i[i]*competitors-D2opt[i]*competitors) -
                                                    c*(2*partSi[i]*D2i[i]*competitors/parai_v[3] - 2*partSopt[i]*D2opt[i]*competitors/para_opt[i,3]))-
                               (1/(2*sqrt(sigmazsq_v[i])))*4*c*(partSopt[i]/para_opt[i,3] - partSi[i]/parai_v[3])*(D2opt[i]*competitors/para_opt[i,3] - D2i[i]*competitors/parai_v[3])*
                               (exp(sigs[1])^2 + exp(sigs[2])^2)*MV_numerator)/sigmazsq_v[i]
    ...
    gr <- function(Theta_est,Theta_known,deal_data,isholiday){
      ...
      parMV_val <- parMV(deal_data=deal_data,sales_estimates=sales_estimates[9,],isholiday)
      ub <- 1
      lb <- 1
      
      grt <- (1/pnorm(deal_min_value))*dnorm(deal_min_value)*parMV_val$parmvc
      #grad_c <- sum(grt[grt<quantile(grt,ub) & grt>quantile(grt,lb)])
      #grad_c <- sum(grt/max(abs(grt)))
      grad_c <- sum(grt)
      grt <- (1/pnorm(deal_min_value))*dnorm(deal_min_value)*parMV_val$parmvbeta0
      #grad_beta0 <- sum(grt[grt<quantile(grt,ub) & grt>quantile(grt,lb)])
      #grad_beta0 <- sum(grt/max(abs(grt)))
      grad_beta0 <- sum(grt)
      grt <- (1/pnorm(deal_min_value))*dnorm(deal_min_value)*parMV_val$parmvbetad
      #grad_betad <- sum(grt[grt<quantile(grt,ub) & grt>quantile(grt,lb)])
      #grad_betad <- sum(grt/max(abs(grt)))
      grad_betad <- sum(grt)
      grt <- (1/pnorm(deal_min_value))*dnorm(deal_min_value)*parMV_val$parmvbetap
      #grad_betap <- sum(grt[grt<quantile(grt,ub) & grt>quantile(grt,lb)])
      #grad_betap <- sum(grt/max(abs(grt)))
      grad_betap <- sum(grt)
      grt <- (1/pnorm(deal_min_value))*dnorm(deal_min_value)*parMV_val$parmvbetawk
      #grad_betawk <- sum(grt[grt<quantile(grt,ub) & grt>quantile(grt,lb)])
      #grad_betawk <- sum(grt/max(abs(grt)))
      grad_betawk <- sum(grt)
      grt <- (1/pnorm(deal_min_value))*dnorm(deal_min_value)*parMV_val$parmvbetacomp
      #grad_betacomp <- sum(grt[grt<quantile(grt,ub) & grt>quantile(grt,lb)])
      #grad_betacomp <-sum(grt/max(abs(grt)))
      grad_betacomp <- sum(grt)
      grt <- (1/pnorm(deal_min_value))*dnorm(deal_min_value)*parMV_val$parmvsigepsilon
      ...
      grt <- (1/pnorm(deal_min_value))*dnorm(deal_min_value)*parMV_val$parmvsigxi
...
get_deal_minimizer <- function(ThetaAll,parai,deal_data,isholiday){
  ...
  if(isholiday==0){
    ...
    gr <- function(parak,ThetaAll,parai,deal_data,isholiday){
      ...
      deriv_vec[1] <- -parMalpha+(c/parak[3])*parSsqalpha
      ...
      deriv_vec[3] <- (-parMd+(c/parak[3])*parSsqd - c*partSk^2/(parak[3]^2))
