...
out_h_pick <- list()
out_h_pick[[1]] <- bs_outputs[[1]][[22]]
out_h_pick[[2]] <- bs_outputs[[2]][[18]]
out_h_pick[[11]] <- bs_outputs[[11]][[20]]
out_h_pick[[5]] <- bs_outputs[[5]][[2]]
out_h_pick[[10]] <- bs_outputs_stored[[10]][[2]]
...
for(i in industries_to_check){
  ...
  if(i==2){
    ...
    hol_data <- dealsfull%>%filter(is.na(holiday)==0,type%in%c(2,3),prep.period>0,offering_duration<=25)%>%{.->>dealsfull_type}%>%
  ...
  print(i)
  ch <- (exp((out_h_pick[[i]]$par[1])))
  cnh <- (exp((mleout_preh[[i]]$par[1])))
  cvec <- c(cvec,ch)
  cnhvec <- c(cnhvec,cnh)
  #print(paste(out_h_pick[[i]]$par[1],mleout_preh[[i]]$par[1]))
  chs <- c(chs, (exp((out_h_pick[[i]]$par[1])))*sd(sdop$original_price))
  cnhs <- c(cnhs,(exp((mleout_preh[[i]]$par[1])))*sd(sdopph$original_price))
  print(paste("mch = ",(exp((out_h_pick[[i]]$par[1])))*sd(sdop$original_price)))
  print(paste("mcph = ",(exp((mleout_preh[[i]]$par[1])))*sd(sdopph$original_price)))
  
  muc <- (exp((out_h_pick[[i]]$par[1])))*sd(sdop$original_price)
  ...
  if(i %in% c(1)){
    ...
    # D1hval[[i]] <- hol_data$total_volume*((out_h_pick[[i]]$par[2])*((1-hol_data$discount)*hol_data$original_price)+((out_h_pick[[i]]$par[3]))*log(hol_data$prep.period)+((out_h_pick[[i]]$par[4]))*log(hol_data$offering_duration)+out_h_pick[[i]]$par[6]+ exp(fes_h)/(hol_data$total_volume-exp(fes_h)))/
    #   (1+((out_h_pick[[i]]$par[2])*((1-hol_data$discount)*hol_data$original_price)+((out_h_pick[[i]]$par[3]))*log(hol_data$prep.period)+((out_h_pick[[i]]$par[4]))*log(hol_data$offering_duration)+out_h_pick[[i]]$par[6]+ exp(fes_h)/(hol_data$total_volume-exp(fes_h))))
    # 
    # D1hvalpri[[i]] <- hol_data$total_volume*(out_h_pick[[i]]$par[6]+ exp(fes_h)/(hol_data$total_volume-exp(fes_h)))/
    #   (1+(out_h_pick[[i]]$par[6]+ exp(fes_h)/(hol_data$total_volume-exp(fes_h))))
    # 
    
    D1hval[[i]] <- Stot*((out_h_pick[[i]]$par[2])*((1-hol_data$discount)*hol_data$original_price)+((out_h_pick[[i]]$par[3]))*log(hol_data$prep.period)+((out_h_pick[[i]]$par[4]))*log(hol_data$offering_duration)+out_h_pick[[i]]$par[6]+ exp(fes_h)/(hol_data$total_volume-exp(fes_h)))/
      (1+((out_h_pick[[i]]$par[2])*((1-hol_data$discount)*hol_data$original_price)+((out_h_pick[[i]]$par[3]))*log(hol_data$prep.period)+((out_h_pick[[i]]$par[4]))*log(hol_data$offering_duration)+out_h_pick[[i]]$par[6]+ exp(fes_h)/(hol_data$total_volume-exp(fes_h))))
    
    D1hvalpri[[i]] <- Stotpri*(((out_h_pick[[i]]$par[4]))*log(hol_data$holiday.len+1)+out_h_pick[[i]]$par[6]+ exp(fes_h)/(hol_data$total_volume-exp(fes_h)))/
      (1+(((out_h_pick[[i]]$par[4]))*log(hol_data$holiday.len+1)+out_h_pick[[i]]$par[6]+ exp(fes_h)/(hol_data$total_volume-exp(fes_h))))
    
    
    r[[i]] <- ((out_h_pick[[i]]$par[2])*((1-hol_data$discount)*hol_data$original_price)+((out_h_pick[[i]]$par[3]))*log(hol_data$prep.period)+((out_h_pick[[i]]$par[4]))*log(hol_data$offering_duration)+out_h_pick[[i]]$par[6]+ exp(fes_h)/(hol_data$total_volume-exp(fes_h)))/
      (1+((out_h_pick[[i]]$par[2])*((1-hol_data$discount)*hol_data$original_price)+((out_h_pick[[i]]$par[3]))*log(hol_data$prep.period)+((out_h_pick[[i]]$par[4]))*log(hol_data$offering_duration)+out_h_pick[[i]]$par[6]+ exp(fes_h)/(hol_data$total_volume-exp(fes_h))))    
    
    thetaalpha <- c(thetaalpha,(out_h_pick[[i]]$par[2]))
    thetaT<- c(thetaT,((out_h_pick[[i]]$par[3])))
    thetad<- c(thetad,((out_h_pick[[i]]$par[4])))
    
    
    
  }else if (i %in%c(2)){
    # D1hval[[i]] <- hol_data$total_volume*((out_h_pick[[i]]$par[2])*((1-hol_data$discount)*hol_data$original_price)+((out_h_pick[[i]]$par[3]))*log(hol_data$prep.period)+(-exp(out_h_pick[[i]]$par[4]))*log(hol_data$offering_duration)+out_h_pick[[i]]$par[6])/
    #   (1+((out_h_pick[[i]]$par[2])*((1-hol_data$discount)*hol_data$original_price)+((out_h_pick[[i]]$par[3]))*log(hol_data$prep.period)+(-exp(out_h_pick[[i]]$par[4]))*log(hol_data$offering_duration)+out_h_pick[[i]]$par[6]))
    # 
    # D1hvalpri[[i]] <- hol_data$total_volume*(+out_h_pick[[i]]$par[6])/
    #   (1+(out_h_pick[[i]]$par[6]))
    
    # r[[i]] <- ((out_h_pick[[i]]$par[2])*((1-hol_data$discount)*hol_data$original_price)+((out_h_pick[[i]]$par[3]))*log(hol_data$prep.period)+(-exp(out_h_pick[[i]]$par[4]))*log(hol_data$offering_duration)+out_h_pick[[i]]$par[6])/
    #   (1+((out_h_pick[[i]]$par[2])*((1-hol_data$discount)*hol_data$original_price)+((out_h_pick[[i]]$par[3]))*log(hol_data$prep.period)+(-exp(out_h_pick[[i]]$par[4]))*log(hol_data$offering_duration)+out_h_pick[[i]]$par[6]))
    # 
    ...
    # D1hvalpri[[i]] <- Stotpri*(((out_h_pick[[i]]$par[3]))*log(hol_data$prep.period)+(-exp(out_h_pick[[i]]$par[4]))*log(hol_data$offering_duration)+out_h_pick[[i]]$par[6] + exp(fes_h)/(hol_data$total_volume-exp(fes_h)))/
    #   (1+(((out_h_pick[[i]]$par[3]))*log(hol_data$prep.period)+(-exp(out_h_pick[[i]]$par[4]))*log(hol_data$offering_duration)+out_h_pick[[i]]$par[6] + exp(fes_h)/(hol_data$total_volume-exp(fes_h))))
    
    D1hvalpri[[i]] <- Stotpri*(((out_h_pick[[i]]$par[4]))*log(hol_data$holiday.len+1)+out_h_pick[[i]]$par[6] + exp(fes_h)/(hol_data$total_volume-exp(fes_h)))/
      (1+(((out_h_pick[[i]]$par[4]))*log(hol_data$holiday.len+1)+out_h_pick[[i]]$par[6] + exp(fes_h)/(hol_data$total_volume-exp(fes_h))))
    
    
    D1hvalprid1[[i]] <- Stot*(((out_h_pick[[i]]$par[3]))*log(2)+out_h_pick[[i]]$par[6] + exp(fes_h)/(hol_data$total_volume-exp(fes_h)))/
      (1+(((out_h_pick[[i]]$par[3]))*log(2)+out_h_pick[[i]]$par[6] + exp(fes_h)/(hol_data$total_volume-exp(fes_h))))
    
    
    r[[i]] <- ((out_h_pick[[i]]$par[2])*((1-hol_data$discount)*hol_data$original_price)+((out_h_pick[[i]]$par[3]))*log(hol_data$prep.period)+((out_h_pick[[i]]$par[4]))*log(hol_data$offering_duration)+out_h_pick[[i]]$par[6])/
      (1+((out_h_pick[[i]]$par[2])*((1-hol_data$discount)*hol_data$original_price)+((out_h_pick[[i]]$par[3]))*log(hol_data$prep.period)+((out_h_pick[[i]]$par[4]))*log(hol_data$offering_duration)+out_h_pick[[i]]$par[6]))
    ...
    D1hval[[i]] <- Stot*((out_h_pick[[i]]$par[2])*((1-hol_data$discount)*hol_data$original_price)+((out_h_pick[[i]]$par[3]))*log(hol_data$prep.period)+(-exp(out_h_pick[[i]]$par[4]))*log(hol_data$offering_duration)+out_h_pick[[i]]$par[6])/
      (1+((out_h_pick[[i]]$par[2])*((1-hol_data$discount)*hol_data$original_price)+((out_h_pick[[i]]$par[3]))*log(hol_data$prep.period)+(-exp(out_h_pick[[i]]$par[4]))*log(hol_data$offering_duration)+out_h_pick[[i]]$par[6]))
    
    D1hvalpri[[i]] <- Stotpri*((-exp(out_h_pick[[i]]$par[4]))*log(hol_data$holiday.len+1)+out_h_pick[[i]]$par[6])/
      (1+((-exp(out_h_pick[[i]]$par[4]))*log(hol_data$holiday.len+1)+out_h_pick[[i]]$par[6]))
    
    D1hvalprid1[[i]] <- Stot*(((out_h_pick[[i]]$par[3]))*log(2)+out_h_pick[[i]]$par[6])/
      (1+(((out_h_pick[[i]]$par[3]))*log(2)+out_h_pick[[i]]$par[6]))
    ...
    D1hval[[i]] <- Stot*((out_h_pick[[i]]$par[2])*((1-hol_data$discount)*hol_data$original_price)+((out_h_pick[[i]]$par[3]))*log(hol_data$prep.period)+((out_h_pick[[i]]$par[4]))*log(hol_data$offering_duration)+out_h_pick[[i]]$par[6])/
      (1+((out_h_pick[[i]]$par[2])*((1-hol_data$discount)*hol_data$original_price)+((out_h_pick[[i]]$par[3]))*log(hol_data$prep.period)+((out_h_pick[[i]]$par[4]))*log(hol_data$offering_duration)+out_h_pick[[i]]$par[6]))
    
    D1hvalpri[[i]] <- Stotpri*(((out_h_pick[[i]]$par[4]))*log(hol_data$holiday.len+1)+out_h_pick[[i]]$par[6])/
      (1+(((out_h_pick[[i]]$par[4]))*log(hol_data$holiday.len+1)+out_h_pick[[i]]$par[6]))
    ...
    D1hval[[i]] <- Stot*((out_h_pick[[i]]$par[2])*((1-hol_data$discount)*hol_data$original_price)+((out_h_pick[[i]]$par[3]))*log(hol_data$prep.period)+(-exp(out_h_pick[[i]]$par[4]))*log(hol_data$offering_duration)+out_h_pick[[i]]$par[6]+ exp(fes_h)/(hol_data$total_volume-exp(fes_h)))/
      (1+((out_h_pick[[i]]$par[2])*((1-hol_data$discount)*hol_data$original_price)+((out_h_pick[[i]]$par[3]))*log(hol_data$prep.period)+(-exp(out_h_pick[[i]]$par[4]))*log(hol_data$offering_duration)+out_h_pick[[i]]$par[6]+ exp(fes_h)/(hol_data$total_volume-exp(fes_h))))
    
    D1hvalpri[[i]] <- Stotpri*((-exp(out_h_pick[[i]]$par[4]))*log(hol_data$holiday.len+1)+out_h_pick[[i]]$par[6]+ exp(fes_h)/(hol_data$total_volume-exp(fes_h)))/
      (1+((-exp(out_h_pick[[i]]$par[4]))*log(hol_data$holiday.len+1)+out_h_pick[[i]]$par[6]+ exp(fes_h)/(hol_data$total_volume-exp(fes_h))))
    
    D1hvalprid1[[i]] <- Stot*(((out_h_pick[[i]]$par[3]))*log(2)+out_h_pick[[i]]$par[6]+ exp(fes_h)/(hol_data$total_volume-exp(fes_h)))/
      (1+(((out_h_pick[[i]]$par[3]))*log(2)+out_h_pick[[i]]$par[6]+ exp(fes_h)/(hol_data$total_volume-exp(fes_h))))
    
    r[[i]] <- ((out_h_pick[[i]]$par[2])*((1-hol_data$discount)*hol_data$original_price)+((out_h_pick[[i]]$par[3]))*log(hol_data$prep.period)+(-exp(out_h_pick[[i]]$par[4]))*log(hol_data$offering_duration)+out_h_pick[[i]]$par[6]+ exp(fes_h)/(hol_data$total_volume-exp(fes_h)))/
      (1+((out_h_pick[[i]]$par[2])*((1-hol_data$discount)*hol_data$original_price)+((out_h_pick[[i]]$par[3]))*log(hol_data$prep.period)+(-exp(out_h_pick[[i]]$par[4]))*log(hol_data$offering_duration)+out_h_pick[[i]]$par[6]+ exp(fes_h)/(hol_data$total_volume-exp(fes_h))))
    
    thetaalpha <- c(thetaalpha,(out_h_pick[[i]]$par[2]))
    thetaT<- c(thetaT,((out_h_pick[[i]]$par[3])))
    thetad<- c(thetad,(-exp(out_h_pick[[i]]$par[4])))
  ...
  if(i %in% c(1)){
    D2hval[[i]] <- exp((out_h_pick[[i]]$par[5])*log(hol_data$offering_duration)+((out_h_pick[[i]]$par[7]))+
                         (D2nhval[[i]]))

    D2hvalpri[[i]] <- exp((out_h_pick[[i]]$par[5])*log(hol_data$holiday.len+1)+((out_h_pick[[i]]$par[7]))+
                            (D2nhvalpri[[i]]))

    D2hvalprid1[[i]] <- exp((out_h_pick[[i]]$par[5])*log(2)+((out_h_pick[[i]]$par[7]))+
                              D2nhvalprid1[[i]])

    betad <- c(betad,(out_h_pick[[i]]$par[5]))

  }else if(i %in% c(10)){
    D2hval[[i]] <- ((out_h_pick[[i]]$par[5])*log(hol_data$offering_duration)+((out_h_pick[[i]]$par[7]))+
                      abs(D2nhval[[i]]))

    D2hvalpri[[i]] <- ((out_h_pick[[i]]$par[5])*log(hol_data$holiday.len+1)+((out_h_pick[[i]]$par[7]))+
                         abs(D2nhvalpri[[i]]))


    D2hvalpri[[i]] <- (((out_h_pick[[i]]$par[7]))+
                         (D2nhvalpri[[i]]))
    ...
    D2hvalpri[[i]] <- exp(((out_h_pick[[i]]$par[7]))+
                            ((D2nhvalpri[[i]])))
  ...
  #print(sd(sdop$original_price)*exp(out_h_pick[[i]]$par[1]))
...
Varidxvec <- vector()
for(i in industries_to_check){
  ch <- (exp(ceiling(out_h_pick[[i]]$par[1])))
