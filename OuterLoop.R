...
sample_loglik_all <- function(Theta_est,Theta_known,deal_data,isholiday){
  # isholiday is the indicator of whether it is for holiday or non-holiday deals calculations
  # Theta <- c(cnh, sigepsilon, sigxi, )
  # parai <- c(alphai, Ti, di)
  # parak <- c(alphak, Tk, dk)
  ...
  single_likelihood <- vector()
  deal_min_value <<- vector()
  deal_argmin_value <<- vector()
    ...
    for(i in 1:dim(deal_data)[1]){
      ...
      tryCatch(
        expr = {minimizer_out <- get_deal_minimizer(ThetaAll,
                                                    parai=c(deal_data[i,"discount"],deal_data[i,"prep.period"],deal_data[i,"offering_duration"]),
                                                    deal_data[i,],isholiday)
        deal_min_value[i] <<- minimizer_out$min_value
        single_likelihood[i] <- pnorm(deal_min_value[i])
        deal_argmin_value <<- rbind(deal_argmin_value,minimizer_out$deal_min$par)},
        error = function(e){
          message("* Caught an error on itertion ", i)
          print(e)
        }
  ...
  # sample_loglik_neg <- -sum(ifelse(log(single_likelihood)==-Inf,
  #                                  min(log(single_likelihood)[(log(single_likelihood)!=-Inf)]),
  #                                  log(single_likelihood)),na.rm = TRUE)
  
  sample_loglik_neg <- -sum(log(single_likelihood))
  print(paste("loop",sample_loglik_neg,sep=" "))
  return(sample_loglik_neg)
...
txtStart("test.txt")
sample_loglik_all(Theta_est,Theta_known,deal_data,isholiday=0) -> tmp
txtStop()
...
MLE_estimation <- function(deal_data,sales_estimates,isholiday){
  if(isholiday==0){
    ...
    Theta_known <- sales_estimates[2,]
    sigs_ini <- c(log(sd(deal_data$total_volume)),log(sd(deal_data$total_volume)))
    Theta_est <- c(0.05,sales_estimates[2,c(1,2,6,9,10)],sigs_ini)
    ...
    fn <- function(Theta_est,Theta_known,deal_data,isholiday){
      rt <- sample_loglik_all(Theta_est,Theta_known,deal_data,isholiday)
      #print(rt)
      #print(Theta_est)
      tryCatch(
        expr = {print(gr(Theta_est,Theta_known,deal_data,isholiday))
          print(Theta_est)},
        error = function(e){
          message("* Caught an error on itertion ", i)
          print(e)
    ...
    parMV <- function(Theta_est,Theta_known,deal_data,sales_estimates,isholiday){
      
      
      ThetaAll_new <- list(Theta_est[1],Theta_known,c(Theta_est[2:6],Theta_known[13:26]),Theta_est[7:8])
      c <- unlist(ThetaAll_new[1])
      sigs <- unlist(ThetaAll_new[4])
      
      para_opt <- deal_argmin_value
      ...
      for(i in 1:dim(deal_data)[1]){
        ...
        D2opt[i] <- D2nhtilde(ThetaAll_new,para_opt[i,],deal_data[i,],isholiday)
        D2i[i] <- D2nhtilde(ThetaAll_new,parai_v,deal_data[i,],isholiday)
        partMopt[i] <- partM(ThetaAll_new,para_opt[i,],deal_data[i,],isholiday)
        partSopt[i] <- partS(ThetaAll_new,para_opt[i,],deal_data[i,],isholiday)
        partSi[i] <- partS(ThetaAll_new,parai_v,deal_data[i,],isholiday)
        partMi[i] <- partM(ThetaAll_new,parai_v,deal_data[i,],isholiday)
        
        MV_numerator <- (partMi[i]-partMopt[i]-c*(partSi[i]^2/parai_v[3] - partSopt[i]^2/para_opt[i,3]))
        
        sigmazsq_v[i] <- sigmazsq(ThetaAll_new,parai_v,para_opt[i,],deal_data[i,],isholiday)
        parsigzc[i] <- (1/(2*sqrt(sigmazsq_v[i])))*8*c*((partSopt[i]/para_opt[i,3] - partSi[i]/parai_v[3])^2)*(exp(sigs[1])^2 + exp(sigs[2])^2)
        
        parmvc[i] <- (sqrt(sigmazsq_v[i])*(partSopt[i]^2/para_opt[i,3] - partSi[i]^2/parai_v[3])-(MV_numerator*parsigzc[i]))/sigmazsq_v[i]
        
        parmvbeta0[i] <- (sqrt(sigmazsq_v[i])*(original_price*(D2i[i] - D2opt[i]) - c*(2*partSi[i]*D2i[i]/parai_v[3] - 2*partSopt[i]*D2opt[i]/para_opt[i,3])) -
                            (1/(2*sqrt(sigmazsq_v[i])))*8*(c^2)*(-partSopt[i]/para_opt[i,3] + partSi[i]/parai_v[3])*(-D2opt[i]/para_opt[i,3] + D2i[i]/parai_v[3])*(exp(sigs[1])^2 + exp(sigs[2])^2)*MV_numerator)/
          sigmazsq_v[i]
        
        parmvbetad[i] <- (sqrt(sigmazsq_v[i])*(original_price*(D2i[i]*log(parai_v[3])-D2opt[i]*log(para_opt[i,3])) -
                                                 c*(2*partSi[i]*D2i[i]*log(parai_v[3])/parai_v[3] - 2*partSopt[i]*D2opt[i]*log(para_opt[i,3])/para_opt[i,3]))-
                            (1/(2*sqrt(sigmazsq_v[i])))*8*(c^2)*(-partSopt[i]/para_opt[i,3] + partSi[i]/parai_v[3])*(-D2opt[i]*log(para_opt[i,3])/para_opt[i,3] + D2i[i]*log(parai_v[3])/parai_v[3])*
        ...
        parmvbetap[i] <- (sqrt(sigmazsq_v[i])*(original_price*(D2i[i]*log(original_price)-D2opt[i]*log(original_price)) -
                                                 c*(2*partSi[i]*D2i[i]*log(original_price)/parai_v[3] - 2*partSopt[i]*D2opt[i]*log(original_price)/para_opt[i,3]))-
                            (1/(2*sqrt(sigmazsq_v[i])))*8*(c^2)*(-partSopt[i]/para_opt[i,3] + partSi[i]/parai_v[3])*(-D2opt[i]*log(original_price)/para_opt[i,3] + D2i[i]*log(original_price)/parai_v[3])*
                            (exp(sigs[1])^2 + exp(sigs[2])^2)*MV_numerator)/sigmazsq_v[i]
        
        parmvbetawk[i] <- (sqrt(sigmazsq_v[i])*(original_price*(D2i[i]*weekends-D2opt[i]*weekends) -
                                                  c*(2*partSi[i]*D2i[i]*weekends/parai_v[3] - 2*partSopt[i]*D2opt[i]*weekends/para_opt[i,3]))-
                             (1/(2*sqrt(sigmazsq_v[i])))*8*(c^2)*(-partSopt[i]/para_opt[i,3] + partSi[i]/parai_v[3])*(-D2opt[i]*weekends/para_opt[i,3] + D2i[i]*weekends/parai_v[3])*
                             (exp(sigs[1])^2 + exp(sigs[2])^2)*MV_numerator)/sigmazsq_v[i]
        
        parmvbetacomp[i] <- (sqrt(sigmazsq_v[i])*(original_price*(D2i[i]*log(competitors)-D2opt[i]*log(competitors)) -
                                                    c*(2*partSi[i]*D2i[i]*log(competitors)/parai_v[3] - 2*partSopt[i]*D2opt[i]*log(competitors)/para_opt[i,3]))-
                               (1/(2*sqrt(sigmazsq_v[i])))*8*(c^2)*(-partSopt[i]/para_opt[i,3] + partSi[i]/parai_v[3])*(-D2opt[i]*log(competitors)/para_opt[i,3] + D2i[i]*log(competitors)/parai_v[3])*
                               (exp(sigs[1])^2 + exp(sigs[2])^2)*MV_numerator)/sigmazsq_v[i]
        
        
        parmvsigepsilon[i] <- (-MV_numerator*(1/(2*sqrt(sigmazsq_v[i])))*(((2*c*(partSopt[i]/para_opt[i,3] - partSi[i]/parai_v[3]))^2)*(2*exp(sigs[1])^2)+
                                 (((parai_v[1]-para_opt[i,1])*original_price)^2)*(2*exp(sigs[1])^2)))/sigmazsq_v[i]
        
        
        #parmvsigepsilon[i] <- 0 
        parmvsigxi[i] <- (-MV_numerator*(1/(2*sqrt(sigmazsq_v[i])))*((2*c*(partSopt[i]/para_opt[i,3] - partSi[i]/parai_v[3]))^2)*(2*exp(sigs[2])^2))/sigmazsq_v[i]
      }
    ...
    gr <- function(Theta_est,Theta_known,deal_data,isholiday){
      ...
      parMV_val <- parMV(Theta_est,Theta_known,deal_data=deal_data,sales_estimates=sales_estimates[2,],isholiday)
      ub <- 1
      lb <- 1

      grt <- ifelse(1/pnorm(deal_min_value)==Inf,0,1/pnorm(deal_min_value))*dnorm(deal_min_value)*parMV_val$parmvc
      #grad_c <- sum(grt[grt<quantile(grt,ub) & grt>quantile(grt,lb)])
      #grad_c <- sum(grt/max(abs(grt)))
      grad_c <- sum(grt)
      grt <- ifelse(1/pnorm(deal_min_value)==Inf,0,1/pnorm(deal_min_value))*dnorm(deal_min_value)*parMV_val$parmvbeta0
      #grad_beta0 <- sum(grt[grt<quantile(grt,ub) & grt>quantile(grt,lb)])
      #grad_beta0 <- sum(grt/max(abs(grt)))
      grad_beta0 <- sum(grt)
      grt <- ifelse(1/pnorm(deal_min_value)==Inf,0,1/pnorm(deal_min_value))*dnorm(deal_min_value)*parMV_val$parmvbetad
      #grad_betad <- sum(grt[grt<quantile(grt,ub) & grt>quantile(grt,lb)])
      #grad_betad <- sum(grt/max(abs(grt)))
      grad_betad <- sum(grt)
      grt <- ifelse(1/pnorm(deal_min_value)==Inf,0,1/pnorm(deal_min_value))*dnorm(deal_min_value)*parMV_val$parmvbetap
      #grad_betap <- sum(grt[grt<quantile(grt,ub) & grt>quantile(grt,lb)])
      #grad_betap <- sum(grt/max(abs(grt)))
      grad_betap <- sum(grt)
      grt <- ifelse(1/pnorm(deal_min_value)==Inf,0,1/pnorm(deal_min_value))*dnorm(deal_min_value)*parMV_val$parmvbetawk
      #grad_betawk <- sum(grt[grt<quantile(grt,ub) & grt>quantile(grt,lb)])
      #grad_betawk <- sum(grt/max(abs(grt)))
      grad_betawk <- sum(grt)
      grt <- ifelse(1/pnorm(deal_min_value)==Inf,0,1/pnorm(deal_min_value))*dnorm(deal_min_value)*parMV_val$parmvbetacomp
      #grad_betacomp <- sum(grt[grt<quantile(grt,ub) & grt>quantile(grt,lb)])
      #grad_betacomp <-sum(grt/max(abs(grt)))
      grad_betacomp <- sum(grt)
      grt <- ifelse(1/pnorm(deal_min_value)==Inf,0,1/pnorm(deal_min_value))*dnorm(deal_min_value)*parMV_val$parmvsigepsilon
      #grad_sigepsilon <- sum(grt[grt<quantile(grt,ub) & grt>quantile(grt,lb)])
      #grad_sigepsilon <- sum(grt/max(abs(grt)))
      grad_sigepsilon <- sum(grt)
      grt <- ifelse(1/pnorm(deal_min_value)==Inf,0,1/pnorm(deal_min_value))*dnorm(deal_min_value)*parMV_val$parmvsigxi
      #grad_sigxi <- sum(grt[grt<quantile(grt,ub) & grt>quantile(grt,lb)])
      #grad_sigxi <- sum(grt/max(abs(grt)))
      grad_sigxi <- sum(grt)
      
      print(c(grad_c,grad_beta0,grad_betad,grad_betap,grad_betawk,grad_betacomp,grad_sigepsilon,grad_sigxi))
    ...
    # txtStart("test.txt")
    # tic()
    # constrOptim(theta = Theta_est, f=sample_loglik_all,
    #             grad = gr,
    #             # grad = grad(func = sample_loglik_all, x=Theta_est,
    #             #             Theta_known = Theta_known, deal_data= deal_data,isholiday=0),
    #             ui = (rbind(c(1,rep(0,7)),c(rep(0,6),1,0),c(rep(0,7),1))), ci = rep(0,3),
    #             Theta_known = Theta_known, deal_data= deal_data,isholiday=0,
    #             #method = "Nelder-Mead",
    ...
    # txtStart("test.txt")
    # tic()
    # maxBFGS(start = Theta_est, fn=fn_max,
    #             grad = gr,
    #             control = list(printLevel=3), 
    #         constraints = list(ineqA = (rbind(c(1,rep(0,7)),c(rep(0,6),1,0),c(rep(0,7),1))), ineqB = rep(0,3)),
    #         Theta_known = Theta_known, deal_data= deal_data,isholiday=0
    # ) -> optimTheta1   
    # toc()
    # txtStop()
...
compareDerivatives(f=fn,grad=gr,hess=NULL,t0=Theta_est,eps=1e-05,Theta_known = Theta_known, deal_data= deal_data,isholiday=0)
...
get_deal_minimizer <- function(ThetaAll,parai,deal_data,isholiday){
  ...
  if(isholiday==0){
    ...
    #   ThetaAll=ThetaAll,parai=parai,deal_data=deal_data,isholiday=0,
    #   method = "BFGS",
    #   control = list(factr = 1e-3)) -> deal_min)
...
get_deal_minimizer(ThetaAll,parai=c(deal_data[1,"discount"],deal_data[1,"prep.period"],deal_data[1,"offering_duration"]),deal_data[1,],isholiday=0) -> tmpp
...
# ipoptr(x0=Theta_est, eval_f = fn, eval_grad_f = gr,
#        ThetaAll=ThetaAll,parai=parai,deal_data=deal_data,isholiday=0)


ipoptr(x0=parak, eval_f = deal_minimizer, eval_grad_f = gr,
       ThetaAll=ThetaAll,parai=parai,deal_data=deal_data[1,],isholiday=0)
