...
dealloglik <- function(Theta_est,Theta_known,para,deal_data,isholiday,IV,normalize,Theta_est_nh){
  ...
  if(isholiday==0){
    ...
    D1barh <- D1bar*((-exp(Theta_est[2])*discprice+((Theta_est[3]))*log(para[,2])+((Theta_est[4]))*log(para[,3])+Theta_est[6])/
                       (1+(-exp(Theta_est[2])*discprice+((Theta_est[3]))*log(para[,2])+((Theta_est[4]))*log(para[,3])+Theta_est[6])))
    D2barh <- ((Theta_est[5])*log(para[,3])+((Theta_est[7]))+log(abs(D2barnh)))
...
sample_loglik <- function(Theta_est,Theta_known,deal_data,isholiday,Theta_est_nh){
  ...
  if(isholiday==0){
    ...
    for(i in 1:dim(deal_data)[1]){
      # print(i)
      total_sales <- deal_data[i,]$total_volume
      D2barnh_t <- exp(Theta_est_nh[2]+((Theta_est_nh[4]))*(1-para_choice[,1])*deal_data$original_price[i]+
                       -exp(Theta_est_nh[3])*log(para_choice[,3]))
      D1hbar_t <- total_sales*(-exp(Theta_est[2])*((1-para_choice[,1])*deal_data$original_price[i])+((Theta_est[3]))*log(para_choice[,2])+((Theta_est[4]))*log(para_choice[,3])+Theta_est[6])/
        (1+(-exp(Theta_est[2])*((1-para_choice[,1])*deal_data$original_price[i])+((Theta_est[3]))*log(para_choice[,2])+((Theta_est[4]))*log(para_choice[,3])+Theta_est[6]))
      D2hbar_t <- ((Theta_est[5])*log(para_choice[,3])+((Theta_est[7]))+log(abs(D2barnh_t)))
