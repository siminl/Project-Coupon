...
getIV <- function(ThetaAll,para,deal_data,D1,D2,isholiday,Theta_est_nh){
  if(isholiday==0){
    ...
    IV_vec <-  ((1-para[,1])*D1bar + original_price*D2barnh -
                  ...
                  original_price*D2 -
                  ((exp(ThetaAll[1]))/hollen)*((D1+D2)^2++exp(2*(ThetaAll[2] + ThetaAll[4]*para[,2]))+exp(2*ThetaAll[2])))/(D1bar)
...
dealloglik <- function(Theta_est,Theta_known,para,deal_data,isholiday,IV,normalize,Theta_est_nh){
  ...
  if(isholiday==0){
    ...
    D1barh <- D1bar*((-exp(Theta_est[2])*discprice+((Theta_est[3]))*log(para[,2])+(-exp(Theta_est[4]))*log(para[,3])+Theta_est[6])/
                       (1+(-exp(Theta_est[2])*discprice+((Theta_est[3]))*log(para[,2])+(-exp(Theta_est[4]))*log(para[,3])+Theta_est[6])))
    D2barh <- exp(-exp(Theta_est[5])*log(para[,3])+((Theta_est[7]))+(abs(D2barnh)))
    numer <- (discprice*D1bar + original_price*D2barnh -
                (exp(cnh)/(para[,3]-hollen))*((D1bar- D1barh+ D2barnh)^2+
                                                exp(2*(ThetaAll[2] + ThetaAll[4]*para[,2])) + exp(2*ThetaAll[3])) +
                original_price*D2barh -
                ((exp(Theta_est[1]))/hollen)*((D1barh+D2barh)^2+exp(2*(ThetaAll[2] + ThetaAll[4]*para[,2]))+exp(2*ThetaAll[2])))/(D1bar)
...
sample_loglik <- function(Theta_est,Theta_known,deal_data,isholiday,Theta_est_nh){
  ...
  if(isholiday==0){
    ...
    for(i in 1:dim(deal_data)[1]){
      # print(i)
      total_sales <- deal_data[i,]$total_volume
      D2barnh_t <- (Theta_est_nh[2]+((Theta_est_nh[4]))*(1-para_choice[,1])*deal_data$original_price[i]+
                         -exp(Theta_est_nh[3])*log(para_choice[,3]))
      D1hbar_t <- total_sales*(-exp(Theta_est[2])*((1-para_choice[,1])*deal_data$original_price[i])+((Theta_est[3]))*log(para_choice[,2])+(-exp(Theta_est[4]))*log(para_choice[,3])+Theta_est[6])/
        (1+(-exp(Theta_est[2])*((1-para_choice[,1])*deal_data$original_price[i])+((Theta_est[3]))*log(para_choice[,2])+(-exp(Theta_est[4]))*log(para_choice[,3])+Theta_est[6]))
      D2hbar_t <- exp(-exp(Theta_est[5])*log(para_choice[,3])+((Theta_est[7]))+(abs(D2barnh_t)))
